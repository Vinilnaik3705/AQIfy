...
def calculate_indian_aqi(pm25: float, pm10: float, no2: float, so2: float, co: float, o3: float) -> float:
    ...
    res = max(indices)
    return max(1.0, min(500.0, float(res)))
                                                ...
                                                if c_dist <= 50.0:
                                                    ...
                                                    if feed_resp.status_code == 200:
                                                        ...
                                                        if feed_data.get("status") == "ok":
                                                            ...
                                                            temp_aq_data = feed_data.get("data", {})

                                                            meas_time = temp_aq_data.get("time", {}).get("v")
                                                            ...
                                                            if meas_time:
                                                                ...
                                                                if age_s > 43200:
                                                                    ...
                                                                    safe_print(f"WAQI resolved station for {city_name} is stale ({age_s}s old). Skipping search resolution.")
        ...
        if city_key == "all":
            ...
            for k in all_keys:
                ...
                if not aq_data and k in openmeteo_batch_data:
                    ...
                    if parent_key in ncr_cities and avg_ncr_ground and avg_ncr_ground > 100.0:

                        ncr_scale = max(0.8, min(1.2, aqi_val / 100.0))

                        aqi_val = max(50.0, min(500.0, avg_ncr_ground * ncr_scale))
                ...
                if not aq_data:
                    ...
                    base_pm25 = rng_fb.uniform(15.0, 45.0)
